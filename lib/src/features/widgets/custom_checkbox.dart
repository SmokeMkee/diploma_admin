...
class CustomCheckbox extends StatefulWidget {
  ...
  ///
  /// Defaults to [ThemeData.secondary].
  ///
  ...
  /// the disabled state, [ThemeData.secondary] is used in the
  ...
  /// also null, then the value of [ThemeData.secondary] with alpha
...
class CustomCheckboxState extends State<CustomCheckbox> with TickerProviderStateMixin, ToggleableStateMixin {
...
class _CheckboxPainter extends ToggleablePainter {
  ...
  void _drawBox(Canvas canvas, Rect outer, Paint paint, BorderSide? side, bool fill) {
    //customization for adding padding to the check
    final newOuter = outer.inflate(4);
    if (fill) {
      canvas.drawPath(shape.getOuterPath(newOuter), paint);
    }
    if (side != null) {
      shape.copyWith(side: side).paint(canvas, newOuter);
    }
  }
